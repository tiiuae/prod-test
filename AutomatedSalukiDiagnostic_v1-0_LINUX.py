...
def accel():
    ...
    nAccIdx = accStr.find("TOPIC: sensor_accel") + 20
    if accStr[nAccIdx] == '3':
        x0i = accStr.find("x: ") + 3
        y0i = accStr.find("y: ") + 3
        z0i = accStr.find("z: ") + 3
...
def orientation():
    for i in range(3):
        ...
        if not (x and y and z):
            ...
            input("Place the Saluki on its side and press [ENTER]. (the side with no connectros or leds) ")
        elif i == 2:
...
try:
    saluki = Serial(port=comport, baudrate=115200, timeout=10)
    print("Successully connected to {}.".format(comport))
    saluki.write('\n'.encode('ascii'))
    b = saluki.read(1)
    # Ignore '\xff'
    while b == b'\xff':
    ...
    print("Error opening serial port. Wrong COM or /dev/ttyUSB?")
    input("Press [ENTER] to try again. ")
    restartApp()
...
try:
    ...
    gpsStaIdx = gpsStr.find("status: ") + 8
    if gpsStr[gpsStaIdx:gpsStaIdx+6] == 'NOT OK':
    ...
    nBarIdx = barStr.find("TOPIC: sensor_baro") + 19
    if barStr[nBarIdx] == '2':
    ...
    if inaStr.find("Device not initialized") > -1:
    ...
    uartIdx = rcStr.find("UART RX bytes: ") + 15
    uartBytes = int(rcStr[uartIdx:].split('\n', 1)[0].strip())
    if uartBytes < 1:
    ...
    print("Serial error! Check trace cable and that there are no other sessions open. ({})".format(comport))
